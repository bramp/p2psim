...
plot(averagegroup(n.Host_Peer_Count(x),rep),averagegroup(n.DHT_GetMessage_Hops_Avg(x),rep),'k-','LineWidth', width);
hold on

%x = find(s.Host_SwarmPeerRatio == ratios(1));
%plot(averagegroup(s.Host_Peer_Count(x),rep),averagegroup(s.DHT_GetMessage_Hops_Avg(x),rep),'k--','LineWidth', width);
...
%title('Average DHT hops for Get Messages');
