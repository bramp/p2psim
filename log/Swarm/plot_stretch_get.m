...
%plot(averagegroup(s.Host_Peer_Count(x),rep),averagegroup(s.DHT_GetMessage_Stretch_Avg(x),rep),'k-','LineWidth', width);


x = find(n.Host_SwarmPeerRatio == ratios(3));
plot(averagegroup(n.Host_Peer_Count(x),rep),averagegroup(n.DHT_GetMessage_Stretch_Avg(x),rep),'k--','LineWidth', width);
hold on
...
%plot(averagegroup(s.Host_Peer_Count(x),rep),averagegroup(s.DHT_GetMessage_Stretch_Avg(x),rep),'k-x','LineWidth', width);

x = find(n.Host_SwarmPeerRatio == ratios(1));
plot(averagegroup(n.Host_Peer_Count(x),rep),averagegroup(n.DHT_GetMessage_Stretch_Avg(x),rep),'k--x','LineWidth', width);

%title('Average DHT Stretch for Get Messages');
