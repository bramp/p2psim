...
if numHeaderRows
    ...
    fid = fopen(filename);
    try
        Data = textscan(fid,'%[^\n]',1,'headerlines',firstLineOffset,'delimiter',delimiter,'bufsize', buffersize);
    catch
    end
    fclose(fid);
    headerLine = Data{1};
...
try
    if numDataCols
		...
		if isequal(delimiter,' ')
	        Data = textscan(fid,formatString,'headerlines',numHeaderRows,'bufsize', buffersize);
		else
	        Data = textscan(fid,formatString,'delimiter',delimiter,'headerlines',numHeaderRows,'bufsize', buffersize);
		end
		if numHeaderCols
...
if wasError && numHeaderChars
    ...
    try
		...
		%treat them as one.
		if isequal(delimiter,' ')
		    Data = textscan(fid,formatString,'headerlines',numHeaderRows,'returnonerror',1,'bufsize', buffersize);
		else
	        Data = textscan(fid,formatString,'delimiter',delimiter,'headerlines',numHeaderRows,'returnonerror',1,'bufsize', buffersize);
		end
		textCharData = Data{1};
...
fid=fopen(filename);
try
    Data = textscan(fid,'%[^\n]',1,'headerlines',numHeaderRows,'delimiter',delimiter,'bufsize', buffersize);
catch
end
fclose(fid);
thisLine = Data{1};
