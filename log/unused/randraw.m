...
if strcmp(runMode, 'genRun')
     ...
     if prod(sampleSize) > 0
          switch lower( distribNameInner )
                    ...
                    elseif min_ab < 1 & max_ab > 1
                         ...
                         while ~isempty( indxs )
                              ...
                              l( [ indxs1( log(u2(indxs1)) < (max_ab-1)*log(1-w(indxs1)) ), ...
                                   indxs2( log(u2(indxs2)) < (min_ab-1)*log(w(indxs2)/t) ) ] ) = true;
                         ...
                         while ~isempty( indxs )
                              ...
                              l( [ indxs1(log(u2(l1)) < (max_ab -1)*log((1-w(l1))/(1-t))), ...
                                   indxs2(log(u2(l2))< (min_ab -1) * log(w(l2)/t)) ] ) = true;
               ...
               case {'cauchy', 'lorentz'}
                    % START cauchy HELP  START lorentz HELP 
                    ...
                    %
                    % EXAMPLES:
                    %  1.   y = randraw('caushy', [], [1 1e5]);
                    %  2.   y = randraw('caushy', [10 1], 1, 1e5);
                    %  3.   y = randraw('caushy', [-10 1.5], 1e5 );
                    %  4.   y = randraw('caushy', [5.1 10.3], [1e5 1] );
                    %  5.   randraw('caushy');                       
                    % END cauchy HELP  END lorentz HELP 
               ...
               case {'chi'}
                    ...
                    %  4.   y = randraw('chi', [-2.1  3.5 4.5], [1e5 1] );
                    %  5.   randraw('chi');                    
                    % END chi HELP
               ...
               case {'chisquare', 'chisq', 'chi2'}
                    ...
                    %
                    % NOTES:
                    %   1. Chi square distribution with r degrees of freedom is sum of r  i.i.d Normal 
               ...
               case {'gig'}
                    ...
                    %
                    % Mean = sqrt( chi / psi ) * besselk(lam+1,sqrt(chi*psi))/besselk(lam,sqrt(chi*psi));
                    % Variance = chi/psi * besselk(lam+2,sqrt(chi*psi))/besselk(lam,sqrt(chi*psi)) - Mean^2;
                    %
                    % PARAMETERS:
                    ...
                    %
                    % NOTES:
                    %    1) GIG(lam, psi, chi) = 1/c * GIG(lam, psi/c, chi*c), for all c>=0
                    %    2) GIG(lam, psi, chi) = sqrt(chi/psi) * GIG(lam, sqrt(psi*chi), sqrt(psi*chi));
                    %    3) GIG(lam, psi, chi) = 1 / GIG(-lam, psi, chi);
                    %   
                    ...
                    %
                    % USAGE:
                    %   randraw('gig', [lam, psi, chi], sampleSize) - generate sampleSize number
                    %         of variates from the Generalized Inverse Gaussian distribution with 
                    %         parameters 'lam', 'psi' and 'chi'
                    ...
                    if h<=1 && b<=1                         
                         ...
                         v = rand( sampleSize );
                         out = a * (v./u);
                         indxs = find( log(u) > (h-1)/2*log(x) - b/4*(x + 1./x) + c );
                         while ~isempty( indxs )
                              indxsSize = size( indxs );
                              u = rand( indxsSize );
                              v = rand( indxsSize );
                              outNew = a * (v./u);
                              l = log(u) <= (h-1)/2*log(x) - b/4*(x + 1./x) + c;
                         ...
                         pm = exp((h-1)/2*log(m) - b/4*(m + (1/m))); % sqrt(hx(m))
                         ...
                         vplus = exp( log(1/pm) + log(1/y1) + (h-1)/2*log(1/y1 + m) - ...
                              b/4*(1/y1 + m + 1/(1/y1 + m)) );
                         vminus = -exp( log(1/pm) + log(-1/y2) + (h-1)/2*log(1/y2 + m) - ...
                              b/4*(1/y2 + m + 1/(1/y2 + m)) );  
                         ...
                         out = z + m;
                         indxs = find( log(u) > (log(1/pm) + (h-1)/2*log(out) - b/4*(out + 1./out)) );
                         ...
                         while ~isempty(indxs),
                              ...
                              outNew = z + m;
                              l = ( log(u) <= (log(1/pm) + (h-1)/2*log(outNew) - b/4*(outNew + 1./outNew)) );
                              out( indxs(l) ) = outNew( l );
                              indxs = indxs( ~l );
