							...
							foreach($failTypes as $failType) {
								foreach($percentages as $percentage) {
									...
									if (!(($percentage == 0 && ($failType == "Stealth" || $failType == "Service")) || (($churnTime > $churnTimes[0]) && $failType == "None"))) {
										passthru("nice -n 19 java -Xms300m -Xmx1400m -classpath ./bin sim.main.Simulator sim.workload.sigcomm.KeysTest Keys-$failType $seed $failType $service $stealth $churnTime $keys $k $gets $getTime $percentage");
									}
								}
							}
						}
