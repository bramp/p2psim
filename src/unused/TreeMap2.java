 ...
 * <pre>
 *     Map m = Collections.synchronizedMap(new TreeMap2(...));
 * </pre><p>
 *
...
{
    /**
     * The Comparator used to maintain order in this TreeMap2, or
     * null if this TreeMap2 uses its elements natural ordering.
     *
     * @serial
     */
     ...
     */
    public TreeMap2(Comparator<? super K> c) {
        this.comparator = c;
    }
     ...
     */
    public TreeMap2(Map<? extends K, ? extends V> m) {
        putAll(m);
    }
    ...
    public K getAfterKey(K key) {
    	Entry<K,V> e = getAfterEntry(key);
    	if (e == null)
    		return null;
    	else
    		return e.key;
     ...
     */
    public Entry<K,V> getAfterEntry(K key) {
        Entry<K,V> p = root;
        if (p==null)
            return null;

        while (true) {
            int cmp = compare((K) key, p.key);
            /*if (cmp == 0) {
                return p.key;
            } else */if (cmp < 0) {
                if (p.left != null)
    ...
    /**
     * Removes the mapping for this key from this TreeMap2 if present.
     *
    ...
    /**
     * Removes all mappings from this TreeMap2.
     */
    public void clear() {
        modCount++;
        size = 0;
    ...
    /**
     * Returns a shallow copy of this <tt>TreeMap2</tt> instance. (The keys and
     * values themselves are not cloned.)
     *
     * @return a shallow copy of this Map.
     */
    public Object clone() {
        TreeMap2<K,V> clone = null;
        try {
            clone = (TreeMap2<K,V>) super.clone();
     ...
     * this <tt>TreeMap2</tt> instance, so changes to this map are reflected in
     ...
     *
     * @return a set view of the keys contained in this TreeMap2.
     */
    public Set<K> keySet() {
        if (keySet == null) {
     ...
     * mapping from the TreeMap2, through the <tt>Iterator.remove</tt>,
	...
	implements SortedMap<K,V>, java.io.Serializable {
        ...
        public boolean containsKey(Object key) {
            return inRange((K) key) && TreeMap2.this.containsKey(key);
        }

        public V get(Object key) {
            if (!inRange((K) key))
                return null;
            return TreeMap2.this.get(key);
        }

        public V put(K key, V value) {
            if (!inRange(key))
                throw new IllegalArgumentException("key out of range");
            return TreeMap2.this.put(key, value);
        }
        ...
        public K firstKey() {
	    TreeMap2.Entry<K,V> e = fromStart ? firstEntry() : getCeilEntry(fromKey);
            K first = key(e);
        ...
        public K lastKey() {
	    TreeMap2.Entry<K,V> e = toEnd ? lastEntry() : getPrecedingEntry(toKey);
            K last = key(e);
        ...
        private class EntrySetView extends AbstractSet<Map.Entry<K,V>> {
            ...
            public int size() {
                if (size == -1 || sizeModCount != TreeMap2.this.modCount) {
                    size = 0;  sizeModCount = TreeMap2.this.modCount;
                    Iterator i = iterator();
                    while (i.hasNext()) {
                        size++;
                        i.next();
            ...
            public boolean contains(Object o) {
                ...
                if (!inRange(key))
                    return false;
                TreeMap2.Entry node = getEntry(key);
                return node != null &&
            ...
            public boolean remove(Object o) {
                ...
                if (!inRange(key))
                    return false;
                TreeMap2.Entry<K,V> node = getEntry(key);
    ...
    private abstract class PrivateEntryIterator<T> implements Iterator<T> {
        private int expectedModCount = TreeMap2.this.modCount;
    ...
    /**
     * Compares two keys using the correct comparison method for this TreeMap2.
     */
    ...
    /**
     * Returns the first Entry in the TreeMap2 (according to the TreeMap2's
     * key-sort function).  Returns null if the TreeMap2 is empty.
     */
    ...
    /**
     * Returns the last Entry in the TreeMap2 (according to the TreeMap2's
     * key-sort function).  Returns null if the TreeMap2 is empty.
     */
    ...
    /**
     * Save the state of the <tt>TreeMap2</tt> instance to a stream (i.e.,
     * serialize it).
     *
     * @serialData The <i>size</i> of the TreeMap2 (the number of key-value
     *             mappings) is emitted (int), followed by the key (Object)
     *             and value (Object) for each key-value mapping represented
     *             by the TreeMap2. The key-value mappings are emitted in
     *             key-order (as determined by the TreeMap2's Comparator,
     *             or by the keys' natural ordering if the TreeMap2 has no
     *             Comparator).
     */
    ...
    /**
     * Reconstitute the <tt>TreeMap2</tt> instance from a stream (i.e.,
     * deserialize it).
     */
     ...
     *
     * It is assumed that the comparator of the TreeMap2 is already set prior
     ...
     * It is assumed that the comparator and size fields of the TreeMap2 are
